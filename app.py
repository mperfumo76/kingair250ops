...
menu = st.sidebar.radio(
    "📌 MENÚ",
    ["🏠 Dashboard", "✈️ Aeronave", "👨‍✈️ Pilotos"]
)
    ...
    try:
        ...
        if df.shape[1] > 10:
            ...
            valores = pd.to_numeric(
                columna_k.astype(str).str.replace(",", ".").str.strip(),
                errors="coerce"
            ).dropna()
...
def buscar_telemetria():
    ...
    try:
        ...
        url = f"https://drive.google.com/embeddedfolderview?id={ID_CARPETA_DRIVE}"
...
try:
    df_pilotos = pd.read_excel("vencimientos.xlsx", sheet_name="Pilotos")
    df_avion = pd.read_excel("vencimientos.xlsx", sheet_name="Avion")

    df_pilotos["Vencimiento"] = pd.to_datetime(df_pilotos["Vencimiento"], errors="coerce").dt.date
    df_avion["Fecha Vence"] = pd.to_datetime(df_avion["Fecha Vence"], errors="coerce").dt.date
